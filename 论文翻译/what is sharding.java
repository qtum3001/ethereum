...
随着加密货币的扩展争论仍在继续，一些潜在的解决方案实际上已经开发了很长一段时间了。具体而言，在以太坊中，以可扩展性为代价将重点放在分散化和安全性上，将分片与实施利益共识一致的应用视为网络可扩展到的急需机制。 应用程序的实际水平，同时仍保持其权力下放和安全性。
...
以太坊区块链的当前状态被称为“全局状态”，是每个人在查看特定实例的区块链时可以看到的。在以太坊实现分片的棘手部分是通过将节点分割成更小的子集，这些子集需要能够处理特定的事务集，同时更新网络状态，同时确保它是有效的。
...
分片用于为区块链平台中的大象提供一些有前途的解决方案，可扩展性。虽然比特币的闪电网络正处于测试阶段，并且到目前为止已经展示了一些非常有希望的进展，但以太坊的解决方案带来了一些独特的挑战，因为它被固定为图灵完整的世界计算机。
...
分片将仅在协议级别直接起作用，因此对于最终用户或dapp开发人员而言，它可能不一定与学习有关。无论如何，以太坊试图为一个庞大的，分散的网络进行状态分割是一项令人印象深刻的努力，如果成功实施，将是一项巨大的成就。
